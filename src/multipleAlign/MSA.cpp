...
int MSA::multiSeqAlign(Alignment* alnPtr, DistMatrix* distMat, vector<int>* seqWeight, AlignmentSteps* progSteps, int iStart)
{
    ...
    // maxid --> maxium value of the score related to i-th sequence
    //           the higher the score, the more relevant to some other seq
    ...
    if (iStart == 0)
    {
        ...
        // aligned --> if the seqs of a step are not all 0, then set aligned to 1 for the first pass
        ...
        for (set = 1; set <= _numSteps; ++set)
        {
            ...
            for (i = 1; i <= _numSeqs; i++)
            {
                // if step[step][seq] != 0 and within the cutoff
